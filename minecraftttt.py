...
def running():
 ...
 while True:
  ...
  mq1=mq[0]
  if mq[1]:
   mymouse.move(int(10*np.sin(mq1)),int(10*np.cos(mq1))) 
  if not (key_q.empty): 
   for k in keyq1:
...
def m(x1,x2,y1,y2,): #視点移動 x1,yiからx2,y2への向きを計算
#def m(x,y):
 global ovec
 x1 = round(x1, 3)
 y1 = round(y1, 3)
 ...
 od = np.linalg.norm(ovec)
 if abs(d - od)<0.05:
  vec = ovec
 radian = np.arctan2(vec[0], vec[1]) 
 #degree = radian * (180 / math.pi)
 distance = np.linalg.norm(vec)
 mouse_q.put_nowait([radian,(threshold <= distance)])
 ovec = vec
